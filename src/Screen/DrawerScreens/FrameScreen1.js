    ...
    useCallback(() => {
      const getText = async () => {
        try {
          ...
          const userDataQuerySnapshot = await getDocs(userDataCollectionRef);
      ...
      const getLastViewedIndex = async () => {
        try {
          const docRef = doc(firestore, 'LastViewedUserIndex', 'currentIndex');
          const docSnap = await getDoc(docRef);
          if (docSnap.exists()) {
            setCurrentIndex(docSnap.data().index);
          }
        } catch (error) {
