...
export default function StudentsScreen() {
  ...
  const getLastViewedIndex = async () => {
    try {
      const lastIndexDocRef = doc(
        firestore,
        'LastViewedUserIndex',
        'lastStudentIndex',
      );
      const docSnap = await getDoc(lastIndexDocRef);
      if (docSnap.exists()) {
        const index = docSnap.data().index;
    ...
    useCallback(() => {
      const fetchStudentBySponsor = async () => {
        try {
          ...
          const q = query(colRef, where('sponsor', '==', userID));
  ...
  return (
    <>
      ...
      {/* Button to update student sponsor */}
